-- =====================================================
-- ZENTRADE - FASE 3: TRADES (VERSIÓN SIMPLIFICADA)
-- EJECUTA ESTE SQL COMPLETO EN SUPABASE SQL EDITOR
-- =====================================================

-- PASO 1: Limpiar objetos existentes (si existen)
-- Primero eliminar funciones
DROP FUNCTION IF EXISTS update_daily_summary() CASCADE;
DROP FUNCTION IF EXISTS calculate_trade_pnl() CASCADE;

-- Luego eliminar tablas (esto elimina automáticamente los triggers)
DROP TABLE IF EXISTS trades CASCADE;
DROP TABLE IF EXISTS instrument_specs CASCADE;

-- =====================================================
-- PASO 2: TABLA INSTRUMENT_SPECS
-- =====================================================
...
-- Índices
CREATE INDEX idx_instrument_specs_symbol ON instrument_specs(symbol);
CREATE INDEX idx_instrument_specs_category ON instrument_specs(category);
...
-- =====================================================
-- PASO 3: TABLA TRADES (VERSIÓN SIMPLIFICADA)
-- =====================================================
CREATE TABLE trades (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_trades_date ON trades(user_id, trade_date);
...
-- =====================================================
-- PASO 4: FUNCIÓN PARA ACTUALIZAR DAILY_SUMMARY
-- =====================================================
...
BEGIN
  ...
  -- Eliminar summary si no hay trades
...
-- RLS en instrument_specs
ALTER TABLE instrument_specs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "All authenticated users can view instrument specs" ON instrument_specs;
